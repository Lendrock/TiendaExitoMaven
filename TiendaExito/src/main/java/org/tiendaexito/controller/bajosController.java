...
public class bajosController implements Initializable {
    ...
    private ObservableList<Producto> listaBajos;
    private int indiceActual = -1;

    @FXML
    private TableView<Producto> tablaProductos;
    @FXML
    private TableColumn colIdProducto, colNombre, colDescripcion,
            colPrecio, colStock, colCategoria;

    @FXML
    private TextField txtIdProducto, txtNombre, txtDescripcion,
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        tablaProductos.getSelectionModel().selectedItemProperty().addListener((obs, oldSelection, newSelection) -> {
            if (newSelection != null) {
                indiceActual = listaBajos.indexOf(newSelection);
    ...
    // Renamed for clarity for this specific controller
    private void cargarBajos(String categoria) {
        listaBajos = FXCollections.observableArrayList(listarProductosPorCategoria(categoria));
        tablaProductos.setItems(listaBajos);
    ...
    private void actualizarBotonesNavegacion() {
        ...
        btnSiguiente.setDisable(indiceActual >= listaBajos.size() - 1 || listaBajos.isEmpty());
    }

    @FXML
    private void mostrarSiguiente() {
        if (indiceActual < listaBajos.size() - 1) {
            indiceActual++;
    ...
    private void buscarProductoUser() {
        ...
        for (Producto p : listarProductosPorCategoria("Bajos")) {
        ...
        listaBajos = FXCollections.observableArrayList(resultadosFiltrados);
        tablaProductos.setItems(listaBajos);
        ...
        if (!listaBajos.isEmpty()) {
            ...
            limpiarCamposDetalle();
            indiceActual = -1;
            System.out.println("No se encontraron resultados para la búsqueda en la categoría Bajos.");
        }
        actualizarBotonesNavegacion();
    }
