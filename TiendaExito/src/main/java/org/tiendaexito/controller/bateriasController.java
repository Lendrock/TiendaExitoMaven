...
public class bateriasController implements Initializable {
    ...
    private ObservableList<Producto> listaBaterias;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        cargarBaterias("Baterias");

        tablaProductos.getSelectionModel().selectedItemProperty().addListener((obs, oldSelection, newSelection) -> {
            if (newSelection != null) {
                indiceActual = listaBaterias.indexOf(newSelection);
    ...
    private void cargarBaterias(String categoria) {
        listaBaterias = FXCollections.observableArrayList(listarProductosPorCategoria(categoria));
        tablaProductos.setItems(listaBaterias);

        if (!listaBaterias.isEmpty()) {
    ...
    private void actualizarBotonesNavegacion() {
        ...
        btnSiguiente.setDisable(indiceActual >= listaBaterias.size() - 1 || listaBaterias.isEmpty());
    }

    @FXML
    private void mostrarSiguiente() {
        if (indiceActual < listaBaterias.size() - 1) {
            indiceActual++;
    ...
    private void buscarProductoUser() {
        ...
        for (Producto p : listarProductosPorCategoria("Baterias")) {
        ...
        listaBaterias = FXCollections.observableArrayList(resultadosFiltrados);
        tablaProductos.setItems(listaBaterias);
        ...
        if (!listaBaterias.isEmpty()) {
            ...
            limpiarCamposDetalle();
            indiceActual = -1;
            System.out.println("No se encontraron resultados para la búsqueda en la categoría Baterias.");
        }
        actualizarBotonesNavegacion();
    }
